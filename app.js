/* ============================================================
   PTSC / KANBUKAI Dashboard - FULL FRONTEND JS
   Features:
   - Login system
   - Add / Edit / Delete
   - Tables with auto-refresh
   - Chatboard
   - PDF: per item, all entries, monthly report
============================================================ */
        ...
        data.slice().reverse().forEach(row=>{
            ...
            tr.innerHTML += `
                <td>
                    ...
                    <button class="btn btn-sm btn-outline-secondary" onclick="generateAllPDF('${sheet}')">All PDF</button>
                </td>
            `;
...
// You can repeat similar functions for Arrivals, Updates, Memo, Training, Pni
...
function closeModal(){
    const x = qs("modal-backdrop");
    if(x) x.remove();
}

async function submitEdit(){
    if(!currentEdit.uid || !currentEdit.sheet){
        alert("No item selected for editing"); return;
    }

    const p = new URLSearchParams({
        sheet: currentEdit.sheet,
        action:"update",
        UID: currentEdit.uid
    });
...
async function generateAllPDF(sheet){
    try{
        const data = await apiFetch(new URLSearchParams({sheet, action:"get"}));
        if(!data.length){ alert("No data to generate PDF"); return; }
        const doc = new jsPDF();
        doc.text(`${sheet} Report`, 20, 20);
        const headers = Object.keys(data[0]);
        const rows = data.map(row=>headers.map(h=>row[h]||""));
        doc.autoTable({startY:30, head:[headers], body:rows});
        doc.save(`${sheet}_AllEntries.pdf`);
    }catch(e){ alert("PDF failed: "+e.message); }
}
...
async function generateMonthlyPDF(){
    ...
    for(const sheet of sheets){
        let data = await apiFetch(new URLSearchParams({sheet, action:"get"}));
        // Include archived
        try{
            const archived = await apiFetch(new URLSearchParams({sheet:"Archive_"+sheet, action:"get"}));
            data = data.concat(archived);
        }catch(e){ console.warn("No archive sheet for", sheet); }

        if(!data.length) continue;

        doc.text(`${sheet} Monthly Report`, 20, yPos);
        yPos += 10;

        const headers = Object.keys(data[0]);
        const rows = data.map(r=>headers.map(h=>r[h]||""));

        doc.autoTable({startY:yPos, head:[headers], body:rows, margin:{top:yPos}});
