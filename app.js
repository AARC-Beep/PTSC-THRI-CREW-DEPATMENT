/* ============================================================
   ...
   Features: login, add, edit, delete, tables, chat,
             dashboard cards, sticky notes, item PDFs, monthly PDF
============================================================ */
...
async function addRow(sheet, params){
    try{
        await apiFetch(new URLSearchParams({sheet, action:"add", ...params}));
...
async function openEditModal(sheet, uid){
    try{
        ...
        for(const k in item){
            ...
            html += `
                <label>${k}</label>
                <input id="edit-${k}" class="form-control" value="${val}">
                <br>
            `;
        }
...
async function loadChat(){
    try{
        ...
        qs("chatboard").innerHTML = "Chat load error";
    }
}
...
function loadStickyNote(){
    const val = localStorage.getItem("sticky-note") || "";
    qs("sticky-text").value = val;
}
qs("sticky-text").addEventListener("input", ()=> {
    localStorage.setItem("sticky-note", qs("sticky-text").value);
});
loadStickyNote();
