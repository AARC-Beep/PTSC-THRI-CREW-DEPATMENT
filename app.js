/* ============================================================
   PTSC / KANBUKAI Dashboard - FULL JS
   Supports: login, add, edit, delete, tables, chat, sticky notes
   Item PDFs, Monthly PDFs (including archived), Dashboard summary
============================================================ */

const GAS_URL = "https://script.google.com/macros/s/AKfycbyHJOMWdg01HTWdV1DoMajJV4oFja2YirfG1K56hnkQskFB9YSzfMGvahax8q0BIf9b/exec"; // replace with your deployed Apps Script URL
...
async function addVesselJoin(){
    try{
        ...
        alert("Added"); toggleForm('join-form'); loadAllData(); loadDashboard();
...
async function addArrivals(){
    try{
        ...
        await apiFetch(params);
        alert("Added"); toggleForm('arrival-form'); loadAllData(); loadDashboard();
    }catch(e){ alert("Add failed: "+e.message); }
}

// (Similarly implement addUpdate, addMemo, addTraining, addPni)
...
// Generate monthly PDF (joining + arrivals + archived)
async function generateMonthlyPDF(){
    try{
        ...
        for(const sheet of sheets){
            const data = await apiFetch(new URLSearchParams({sheet, action:"get"}));
            const archiveData = await apiFetch(new URLSearchParams({sheet:"Archive_"+sheet, action:"get"})).catch(()=>[]);
            const rows = [...data,...archiveData];
            if(rows.length===0) continue;
            doc.text(`${sheet} Monthly Report`,20,y); y+=10;
            const tableData = rows.map(r=>Object.entries(r).map(([k,v])=>[k,String(v)]));
            doc.autoTable({startY:y, head:[["Field","Value"]], body:tableData.flat()});
...
// ---------------------- STICKY NOTE ----------------------
const sticky = qs("sticky-text");
sticky.value = localStorage.getItem("sticky-note")||"";
sticky.addEventListener("input",()=>{ localStorage.setItem("sticky-note",sticky.value); });

// ---------------------- FORM TOGGLE ----------------------
function toggleForm(id){ const f=qs(id); f.style.display=(f.style.display==="none")?"block":"none"; }
