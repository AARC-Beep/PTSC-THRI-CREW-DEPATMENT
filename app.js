/* ============================================================
   ...
   Supports: login, add, edit, delete, tables, chat,
             individual item PDFs, monthly PDF
============================================================ */
...
async function loginUser(){
    ...
    const err = qs("login-error"); err.innerText="";
    ...
    try{
        ...
        if(!match){ err.innerText="Invalid username/password"; return; }
...
async function addVesselJoin(){
    try{
        ...
        await apiFetch(params); alert("Added");
...
// Similar add functions for Updates, Memo, Training, Pni...
// addUpdate(), addMemo(), addTraining(), addPni() - follow same pattern
...
async function openEditModal(sheet, uid){
    try{
        ...
        html+=`<button class="btn btn-primary" onclick="submitEdit()">Save</button>`;
        showModal(html);
...
async function submitEdit(){
    ...
    try{
        await apiFetch(p); alert("Updated"); closeModal(); loadAllData(); loadDashboard();
    }catch(e){ alert("Update failed: "+e.message); }
}

// -------------------- DELETE --------------------
function deleteRowConfirm(sheet, uid){ if(confirm("Delete this item?")) deleteRow(sheet,uid); }
...
// Monthly PDF for entire sheet
async function generateMonthlyPDF(){
    const month = new Date().getMonth();
    const doc = new jsPDF();
    let y = 20;
    for(const sheet of ["Vessel_Join","Arrivals","Updates","Memo","Training","Pni"]){
        const data = await apiFetch(new URLSearchParams({sheet,action:"get"}));
        const rows = data.filter(r=>new Date(r.Timestamp).getMonth()===month).map(r=>Object.entries(r).map(([k,v])=>[k,String(v)]));
        if(rows.length){
            doc.text(sheet,20,y); y+=10;
            doc.autoTable({startY:y, head:[["Field","Value"]], body:rows.flat()});
            y+=rows.length*10+10;
        }
    }
